 ...
 * @modify date 2021-09-05 19:14:35
 */
...
vector<int> dijkstraAdjList(int source, vector<vector<pair<int, int>>> graph){
    ...
    while(!vertexSet.empty()){
        ...
        // * relaxing all neighbors of the vertex.0
...
vector<int> dijkistraAdjMatrix(int source, vector<vector<int>> graph){
    ...
    for (int i = 0; i < vertexCount; ++i){
        // * Finding out the vertex which is currently at minimum distance from source.
        // * this will be used for relaxation.
...
int main(){
    ...
    cout << "Answer 2 O(v^2) : \n";
    for (int i = 1; i <= n; ++i){
        cout << "Vertex " << i << " is at a distance of " << answer2[i] << endl;
    }
}